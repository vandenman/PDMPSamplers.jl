@testset "Sampling" begin
    ...
    @testset "Basic PDMP Sampler Tests" begin
        ...
        pdmp_types = (ZigZag, BouncyParticle, Boomerang,
            # Skip PreconditionedZigZag and PreconditionedBPS for now - they need special handling
                    ...
                    @testset "$(data_name(data_type, data_arg))" for data_type in data_types, data_arg in data_args[data_type]
                        ...
                        if algorithm === ThinningStrategy
                            ...
                            if gradient_type === CoordinateWiseGradient
                                ...
                                # For Boomerang with a Gaussian target where the reference matches the target,
                                # the corrected gradient is 0, so c0 should be 0.
                                # For other dynamics or non-matching references, use a small c0.
                                if pdmp_type <: Union{Boomerang,PreconditionedDynamics{<:Any,Boomerang}} && data_type <: Distributions.AbstractMvNormal
                                    # Reference matches target, corrected gradient is 0
                                    c0 = 0.0
                                ...
                                alg = ThinningStrategy(GlobalBounds(c0 / max(d, 1), d))
                            end
            ...
            @testset "$gradient_type" for gradient_type in get_gradient_types(pdmp_type)
                @testset "$algorithm" for algorithm in get_algorithm_types(pdmp_type, gradient_type)
                    ...
                    @testset "$(data_name(data_type, data_arg))" for data_type in data_types, data_arg in data_args[data_type]
                        ...
                        # Deterministic seed per test configuration for reproducibility
                        Random.seed!(hash((pdmp_type, gradient_type, algorithm, data_type, data_arg)))
    ...
    if false
        @testset "Subsampling" begin
            ...
            @test mc_est ≈ true_grad rtol = 0.1
            ...
            model_full = PDMPModel(d, grad_full, ∇²f!)
            alg_full = GridThinningStrategy(N=10)
            trace_full, stats_full = pdmp_sample(ξ0, flow, model_full, alg_full, 0.0, T, progress=show_progress)
            ...
            model_sub = PDMPModel(d, grad, ∇²f!)
            trace, stats = pdmp_sample(ξ0, flow, model_sub, alg, 0.0, T, progress=show_progress)
            ...
            if data_type <: GaussianMeanModel
                ...
                for i in axes(qs_full, 2)
                    ...
                    @test qs_full[:, i] ≈ qs_sub[:, i] rtol = 0.25 atol = 0.1
                end
