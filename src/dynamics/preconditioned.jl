...
function rate_derivatives_for_grid!(
    ...
    return _diagonal_preconditioned_zigzag_rate_derivatives_for_grid_unchecked!(
        values, derivatives, provider, state, flow, t_grid, n_points, x)
end

function _diagonal_preconditioned_zigzag_rate_derivatives_for_grid_unchecked!(
    values::AbstractMatrix,
    derivatives::AbstractMatrix,
    provider::Union{Tuple,GradHVPProvider},
    state::AbstractPDMPState,
...
function rate_derivatives_for_grid!(
    ...
    size(derivatives, 1) >= n_channels && size(derivatives, 2) >= n_points ||
        throw(ArgumentError("derivatives matrix is too small"))
    x = Vector{Float64}(undef, length(x0))
    return _dense_preconditioned_zigzag_rate_derivatives_for_grid_unchecked!(
        values, derivatives, provider, state, flow, t_grid, n_points, x)
end

function _dense_preconditioned_zigzag_rate_derivatives_for_grid_unchecked!(
    values::AbstractMatrix,
    derivatives::AbstractMatrix,
    provider::Union{Tuple,GradHVPProvider},
    state::AbstractPDMPState,
